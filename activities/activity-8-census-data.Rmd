...
```{r, eval = FALSE}
or_county %>% mutate(perc_white = white_popE/total_popE) %>% 
  st_as_sf() %>%
  ggplot() +
  geom_sf(aes(fill = perc_white)) +
  scale_fill_viridis_c() +
  labs(fill = "% of Population that is White") +
  theme_void()
...
```{r}
mplstracts <- get_acs(geography = "tract", 
              year = 2022,
              variables = c(housingvalues = "B25077_001"), 
              state = "MN",
              county = c("Hennepin", "Ramsey", "Dakota", "Anoka", "Carver"),
              survey = "acs5",
              output = "wide")
mn_sf <- tracts(state = "MN", cb = TRUE)
mn_county <- left_join(mplstracts, mn_sf, by = c("GEOID" = "GEOID"))
glimpse(mn_county)

mn_county %>% 
  st_as_sf() %>%
  ggplot() +
  geom_sf(aes(fill = housingvaluesE)) +
  scale_fill_viridis_c() +
  labs(fill = "Median Home Values") +
  theme_void()
