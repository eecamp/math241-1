...
```{r}
pop_table <- html_table(tables[[4]], fill = TRUE)
pop_table

pop_table$Pop. <- as.numeric(gsub(",", "", pop_table$Pop.))


ggplot(pop_table, aes(x= Census, y = Pop.))+
  geom_point()+
...
#Grab the specific table
college_access_table <- html_table(tables2[[1]], fill = TRUE)
...
```{r}
college_access_table$`Net price, mid. income` <- gsub("$", "", college_access_table$`Net price, mid. income`)
